...
async function getOhlcvDay({ day, force = false, max, onProgress } = {}) {
  ...
  let done = 0;
  const rows = await mapPool(symbols, 12, async (sym) => {
    try {
      const chart = await fetchYahooChart(sym, "1mo", "1d");
      done++;
  ...
  const globalSyms = new Set(GLOBALS.map((g) => g.symbol));
