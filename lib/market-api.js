...
/**
 * Refresh universe: merge remote seeds + optional Yahoo validate.
 *
 * Modes:
 *  - quick  : seed merge only (fast, default for UI) — no Yahoo spam
 *  - sample : validate first N tickers (default N=80), keep rest unvalidated
 *  - full   : validate all (slow; can take minutes — use sparingly)
 *
 * Safety: if Yahoo rejects almost everything (rate-limit/network), keep previous
 * list instead of wiping the universe.
 */
async function refreshUniverse({
  ...
  const current = fs.existsSync(UNIVERSE_PATH)
    ...
    : { tickers: [] };
  const prevTickers = (current.tickers || []).map((t) =>
    String(t).toUpperCase().replace(/\.JK$/i, "")
  );
  const set = new Set(prevTickers);

  // Resolve mode
  let resolvedMode = mode;
  if (!resolvedMode) {
    if (!validate) resolvedMode = "quick";
    else if (maxValidate === 0) resolvedMode = "full";
    else resolvedMode = "sample";
  }
  ...
  for (const url of UNIVERSE_SEED_URLS) {
    try {
      ...
      seedHits.push({ url: url.slice(0, 80), added: list.length });
    } catch (e) {
      seedHits.push({ url: url.slice(0, 80), error: String(e.message || e) });
    }
  }

  let tickers = [...set]
    .map((t) => String(t || "").toUpperCase().replace(/\.JK$/i, "").replace(/[^A-Z]/g, ""))
    .filter((t) => /^[A-Z]{2,6}$/.test(t))
    .sort();
  const removed = [];
  const kept = [];
  let validateRan = false;
  let yahooSuspectDown = false;

  const doValidate = resolvedMode === "sample" || resolvedMode === "full";
  if (doValidate && tickers.length) {
    validateRan = true;
    let toCheck = tickers;
    const limit =
      resolvedMode === "full"
        ...
        : maxValidate > 0
          ? maxValidate
          : 80;
    if (limit > 0) toCheck = tickers.slice(0, limit);

    // Tight Yahoo probe: short timeout, small concurrency to avoid browser/server hang
    const checks = await mapPool(toCheck, 6, async (t) => {
      try {
        const chart = await fetchYahooChartFast(`${t}.JK`);
    ...
    // If <15% of checked pass, Yahoo is likely blocked/rate-limited — don't wipe list
    if (toCheck.length >= 20 && okN / toCheck.length < 0.15) {
      yahooSuspectDown = true;
      // keep all tickers (seed merge only)
    } else if (limit > 0 && limit < tickers.length) {
      const unchecked = tickers.slice(limit);
      for (const c of checks) {
        if (c.ok) kept.push(c.t);
        else removed.push(c.t);
      }
      tickers = [...new Set([...kept, ...unchecked])].sort();
    } else {
      for (const c of checks) {
        if (c.ok) kept.push(c.t);
        else removed.push(c.t);
      }
      // Don't allow empty wipe
      if (kept.length >= 50 || kept.length >= prevTickers.length * 0.3) {
        tickers = kept.sort();
      } else {
        yahooSuspectDown = true;
      }
  ...
  // Final safety: never write empty universe if we had one
  if ((!tickers || tickers.length < 30) && prevTickers.length >= 30) {
    tickers = [...new Set([...prevTickers, ...tickers])].sort();
    yahooSuspectDown = true;
  }
  ...
  const out = {
    ...
    note:
      "Refreshed free path (seed + optional Yahoo). Not official IDX dump." +
      (yahooSuspectDown ? " Yahoo probe weak — kept prior tickers." : ""),
    ...
    meta: {
      ...
      validatedCount: validateRan ? kept.length + removed.length : 0,
      validate: validateRan,
      yahooSuspectDown
    }
  };
...
/** Fast Yahoo chart for universe probe only (5d, short timeout). */
async function fetchYahooChartFast(symbol) {
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(
    symbol
  ...
  for (let i = 0; i < ts.length; i++) {
    const c = q.close?.[i];
    if (c == null) continue;
    bars.push({ t: ts[i], c, v: q.volume?.[i] ?? 0 });
  }
  return { bars };
}
...
async function getTickerPack(code, { force = false } = {}) {
  const { parseTicker } = require("./ticker-util.js");
  const parsed = parseTicker(code);
  if (!parsed.ok) throw new Error(parsed.error || "ticker tidak valid");
